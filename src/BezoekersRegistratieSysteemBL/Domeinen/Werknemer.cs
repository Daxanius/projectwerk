...
namespace BezoekersRegistratieSysteemBL.Domeinen {
	public class Werknemer {
		...
		private Dictionary<Bedrijf, WerknemerInfo> werknemerInfo = new Dictionary<Bedrijf, WerknemerInfo>();
		...
		/// <summary>
		/// Zet status naam per werknemer bedrijf.
		/// </summary>
		/// <param name="bedrijf">Bedrijf waar werknemer zijn statusbedrijf gezet moet worden.</param>
		/// <param name="statusNaam">Status naam die gezet meot worden.</param>
		public void ZetStatusNaamPerBedrijf(Bedrijf bedrijf, string statusNaam) {
			if (bedrijf is null) {
				throw new WerknemerException("Bedrijf is leeg");
			}
			if (String.IsNullOrWhiteSpace(statusNaam)) {
				throw new WerknemerException("Status naam is leeg");
			}
			if (!werknemerInfo.ContainsKey(bedrijf)) {
				throw new WerknemerException("Werknemer werkt niet bij deze bedrijf.");
			}
			werknemerInfo[bedrijf].ZetStatusNaam(statusNaam);
		}
