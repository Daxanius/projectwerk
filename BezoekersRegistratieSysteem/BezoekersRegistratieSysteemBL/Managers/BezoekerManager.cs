...
namespace BezoekersRegistratieSysteemBL.Managers
{
	public class BezoekerManager
	{
		...
		{
			if (_bezoekerRepository.BestaatBezoeker(bezoeker))
				throw new BezoekerManagerException("BezoekerManager - VoegBezoekerToe - bezoeker bestaat al");
			try
			{
		...
		{
			if (!_bezoekerRepository.BestaatBezoeker(bezoeker))
				throw new BezoekerManagerException("BezoekerManager - VerwijderBezoeker - bezoeker bestaat niet");
			try
			{
		...
		{
			if (bezoeker == null)
				throw new BezoekerManagerException("BezoekerManager - WijzigBezoeker - bezoeker mag niet leeg zijn");
			if (!_bezoekerRepository.BestaatBezoeker(bezoeker))
				throw new BezoekerManagerException("BezoekerManager - WijzigBezoeker - bezoeker bestaat niet");
			if (_bezoekerRepository.GeefBezoeker(bezoeker.Id).BezoekerIsGelijk(bezoeker))
				throw new BezoekerManagerException("BezoekerManager - WijzigBezoeker - bezoeker is niet gewijzigd");
			try
			{
		...
		public Bezoeker GeefBezoeker(long id)
		{
			if (!_bezoekerRepository.BestaatBezoeker(id))
				throw new BezoekerManagerException("BezoekerManager - GeefBezoeker - bezoeker bestaat niet");
			try
			{
		...
		public IReadOnlyList<Bezoeker> GeefBezoekerOpNaam(string voornaam, string achternaam)
		{
			if (string.IsNullOrWhiteSpace(voornaam) || string.IsNullOrWhiteSpace(achternaam))
				throw new BezoekerManagerException("BezoekerManager - GeefBezoekerOpNaam - naam mag niet leeg zijn");
			try
			{
